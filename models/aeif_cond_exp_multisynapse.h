...
namespace nest
{
  ...
  public:        
	...
	nest::port send_test_event(Node&, rport receptor_type, synindex, bool);
    nest::port handles_test_event(SpikeEvent&, rport);
    nest::port handles_test_event(CurrentEvent&, rport);
    nest::port handles_test_event(DataLoggingRequest&, rport);
  ...
  private:
    ...
    struct Parameters_
    {
      ...
      double AMPA_E_rev;        //!< AMPA reversal Potential in mV
      double AMPA_Tau_decay;    //!< Synaptic Time Constant AMPA Synapse in ms
      double AMPA_NEG_E_rev;        //!< NMDA reversal Potential in mV
      double NMDA_E_rev;        //!< NMDA reversal Potential in mV
      double NMDA_Tau_decay;    //!< Synaptic Time Constant NMDA Synapse in ms
      double NMDA_NEG_E_rev;        //!< NMDA reversal Potential in mV
      double GABA_E_rev;        //!< GABAA reversal Potential in mV
      double GABA_Tau_decay;    //!< Rise Time Constant GABAA Synapse in ms
  ...
  public:
	...
	STATE_VEC_SIZE
      ...
      double I_AMPA_;    //!< AMPA current; member only to allow recording
      double I_AMPA_NEG_;    //!< NMDA current; member only to allow recording
      double I_NMDA_;    //!< NMDA current; member only to allow recording
      double I_NMDA_NEG_;    //!< NMDA current; member only to allow recording
      double I_GABA_; //!< GABAA current; member only to allow recording
    ...
    struct Variables_
    {
      double PSConInit_AMPA;
      double PSConInit_NMDA;
      double PSConInit_NMDA_NEG;
      double PSConInit_AMPA_NEG;
      double PSConInit_GABA;
    ...
    double get_I_NMDA_NEG_() const { return S_.I_NMDA_NEG_;  }
    double get_I_AMPA_NEG_() const { return S_.I_AMPA_NEG_;  }
    double get_I_GABA_() const { return S_.I_GABA_;  }
    double get_bias_() const { return S_.bias; }
    double get_epsilon_() const { return S_.epsilon; }
    double get_kappa_() const { return S_.kappa; }
