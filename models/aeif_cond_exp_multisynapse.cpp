...
int nest::aeif_cond_exp_multisynapse_dynamics (double, const double y[], double f[], void* pnode)
{
  ...
  //const double I_AMPA = -y[S::G_AMPA] * ( V - node.P_.AMPA_E_rev );
  //const double I_NMDA = -y[S::G_NMDA] * ( V - node.P_.NMDA_E_rev );
  //const double I_NMDA_NEG = -y[S::G_NMDA_NEG] * ( V - node.P_.NMDA_NEG_E_rev );
  //const double I_AMPA_NEG = -y[S::G_AMPA_NEG] * ( V - node.P_.AMPA_NEG_E_rev );
  //const double I_GABA = -y[S::G_GABA] * ( V - node.P_.GABA_E_rev );
  ...
  const double I_syn =  node.S_.I_AMPA_ + node.S_.I_NMDA_ + node.S_.I_NMDA_NEG_ + node.S_.I_AMPA_NEG_ + node.S_.I_GABA_ + node.B_.I_stim_;
  ...
  const double I_spike = (exp_arg>10.)? largest_exp : node.P_.Delta_T * std::exp(exp_arg);
...
void nest::aeif_cond_exp_multisynapse::Parameters_::get(DictionaryDatum &d) const
{
  ...
  def<double>(d, "AMPA_Tau_decay",     AMPA_Tau_decay);
  def<double>(d, "NMDA_E_rev",         NMDA_E_rev);
  def<double>(d, "NMDA_NEG_E_rev",     NMDA_NEG_E_rev);
  def<double>(d, "AMPA_NEG_E_rev",     AMPA_NEG_E_rev);
  def<double>(d, "NMDA_Tau_decay",     NMDA_Tau_decay);
  def<double>(d, "GABA_E_rev",         GABA_E_rev);
  def<double>(d, "GABA_Tau_decay",     GABA_Tau_decay);
  def<double>(d, "tau_j",      tau_j);
  def<double>(d, "tau_e",      tau_e);
  def<double>(d, "tau_p",      tau_p);
  def<double>(d, "kappa",      kappa);
  def<double>(d, "bias",      bias);
  def<double>(d, "gain",      gain);
  def<double>(d, "fmax",      fmax);
...
void nest::aeif_cond_exp_multisynapse::Parameters_::set(const DictionaryDatum &d)
{
  ...
  updateValue<double>(d, "AMPA_Tau_decay",    AMPA_Tau_decay);
  updateValue<double>(d, "NMDA_E_rev",        NMDA_E_rev);
  updateValue<double>(d, "NMDA_NEG_E_rev",        NMDA_NEG_E_rev);
  updateValue<double>(d, "AMPA_NEG_E_rev",        AMPA_NEG_E_rev);
  updateValue<double>(d, "NMDA_Tau_decay",    NMDA_Tau_decay);
  updateValue<double>(d, "GABA_E_rev",     GABA_E_rev);
  updateValue<double>(d, "GABA_Tau_decay", GABA_Tau_decay);
  updateValue<double>(d, "tau_j",      tau_j);
  updateValue<double>(d, "tau_e",      tau_e);
  updateValue<double>(d, "tau_p",      tau_p);
  updateValue<double>(d, "kappa",      kappa);
  updateValue<double>(d, "gain",      gain);
  updateValue<double>(d, "bias",      bias);
  updateValue<double>(d, "fmax",      fmax);
...
void nest::aeif_cond_exp_multisynapse::handle(nest::SpikeEvent &e)
{
  ...
  if (e.get_rport() == AMPA - MIN_SPIKE_RECEPTOR && e.get_weight() >= 0)
      B_.spikes_AMPA_.add_value(e.get_rel_delivery_steps(kernel().simulation_manager.get_slice_origin()), e.get_weight() * e.get_multiplicity() );

  else if (e.get_rport() == AMPA - MIN_SPIKE_RECEPTOR && e.get_weight() < 0)
      B_.spikes_AMPA_NEG_.add_value(e.get_rel_delivery_steps(kernel().simulation_manager.get_slice_origin()), -e.get_weight() * e.get_multiplicity() );

  // If NMDA
  else if (e.get_rport() == NMDA - MIN_SPIKE_RECEPTOR && e.get_weight() >= 0)
           B_.spikes_NMDA_.add_value(e.get_rel_delivery_steps(kernel().simulation_manager.get_slice_origin()), e.get_weight() * e.get_multiplicity() );

  else if (e.get_rport() == NMDA - MIN_SPIKE_RECEPTOR && e.get_weight() < 0)
           B_.spikes_NMDA_NEG_.add_value(e.get_rel_delivery_steps(kernel().simulation_manager.get_slice_origin()), -e.get_weight() * e.get_multiplicity() );  
